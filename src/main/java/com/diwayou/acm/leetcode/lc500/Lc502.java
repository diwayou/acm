...
public class Lc502 {
    ...
    private int findMaximizedCapitalHelper(int k, int w, int[] profits, int[] capital, int i) {
        if (k < 0) {
            return 0;
        }

        int max = profits[i], t;
        for (int j = i + 1; j < capital.length; j++) {
            if (capital[j] <= w) {
                t = profits[j];
                t += findMaximizedCapitalHelper(k - 1, w + t, profits, capital, j);
            }
        }
