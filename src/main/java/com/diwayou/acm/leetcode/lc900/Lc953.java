...
public class Lc953 {
    ...
    public boolean isAlienSorted(String[] words, String order) {
        ...
        for (int i = 0; i < words.length - 1; i++) {
            if (greater(words[i], words[i + 1], m)) {
                return false;
            }
        }
    ...
    private boolean greater(String w1, String w2, int[] m) {
        int l = Math.min(w1.length(), w2.length());
        int i;
        for (i = 0; i < l; i++) {
            if (m[w1.charAt(i) - 'a'] > m[w2.charAt(i) - 'a']) {
                return true;
            } else if (m[w1.charAt(i) - 'a'] < m[w2.charAt(i) - 'a']) {
                return false;
            }
        }
