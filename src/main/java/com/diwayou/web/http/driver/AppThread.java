...
public class AppThread {
                ...
                synchronized (statusCode) {
                    if (statusCode.get() == 0) {
                        ...
                        if (statusCode.get() > 299) {
                            log.info("Performing browser action, but HTTP status is " + statusCode.get() + ".");
                        }
                        T result = null;
                        try {
    ...
    public static <T> T exec(final Sync<T> action, long timeout) {
        return exec(false, new StatusCode(), timeout, action);
    }

    public static <T> T exec(final StatusCode statusCode, final Sync<T> action) {
        return exec(false, statusCode, 0, action);
    }

    public static <T> T exec(final boolean pauseAfterExec, final StatusCode statusCode, final Sync<T> action) {
        return exec(pauseAfterExec, statusCode, 0, action);
    }

    public static <T> T exec(final StatusCode statusCode, final long timeout, final Sync<T> action) {
        return exec(false, statusCode, timeout, action);
    }

    public static <T> T exec(final boolean pauseAfterExec, final StatusCode statusCode, final long timeout,
                             final Sync<T> action) {
        try {
